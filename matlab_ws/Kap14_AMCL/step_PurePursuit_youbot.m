function [v_x,v_y,omega] = step_PurePursuit_youbot(pp, cpose, orie, kv_rot, kv_ang)
%%%%------------PurePursuit algorithmus angepasst auf den Kuka-Youbot--------------%%%%
%%%%-------------------Westfälische hochschule Juni/Juli 2017----------------------%%%%
...
%%%%------------Begriffe--------------%%%%
%WP1: Wegpunkt der näher am Startpunkt als am Zielpunkt liegt

%%%%------------Optionale Parameter, Default Werte--------------%%%%
if nargin < 3
    %Verstärkungsfaktor gibt an wie Stark die Rotation geregelt wird
    orie = 0;
end
if nargin < 4
    %Verstärkungsfaktor gibt an wie Stark die Rotation priorisiert wird
    kv_rot = 5;
end
if nargin < 5
    %Verstärkungsfaktor gibt an wie Stark die Rotation geregelt wird
...
wp_spnr=1;
%Pfad Rückwärts durchgehen bis Schnittpunkt(e) gefunden
for i = (length(wp)-1):-1:s(2)
    v_pfad = wp(i+1,:)-wp(i,:);
    if i == length(wp)-1
        %Letzen Pfad-V verlängern damit Look-Ahead auch am Schluss möglich ist
...
%Reglung Priorität der Rotation
...
v_pfad = p2-p1;
s = NaN;
%Senkrechte Pfad/CPose berechen und nötige Streckung bis zum Schnittpunkt
%bestimmen
